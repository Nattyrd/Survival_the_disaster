...
class Boss1 {
    ...
    static createAnimations(scene) {
        const bodyAnims = [
            ...
            { anim: "boss1_walk_attack", sheet: "walk_attack", frames: 6, rate: 12, repeat: 0 },
            ...
            { anim: "boss1_fly_down", sheet: "fly_down", frames: 6, rate: 11, repeat: 0 }
        ];

        const projAnims = [
            { anim: "boss1_proj_special1", sheet: "special1", frames: 6, rate: 16, repeat: -1 },
            { anim: "boss1_proj_special2", sheet: "special2", frames: 4, rate: 14, repeat: -1 }
        ];
    ...
    spawnProjectile(type, player, stats) {
        ...
        const damage = isType1 ? 14 + Math.floor(this.getAggression() * 4) : 20 + Math.floor(this.getAggression() * 6);
    ...
    fireAtPlayer(player, moveWhileShooting) {
        ...
        if (moveWhileShooting) {
            ...
            this.isChasing = true;
            this.scene.physics.moveToObject(this.sprite, player.sprite, stats.walkAttackSpeed);
        } else {
            this.stopMovement();
    ...
    reposition(player, goUp) {
        ...
        const targetY = goUp
            ? Phaser.Math.Clamp(this.sprite.y - Phaser.Math.Between(80, 140), margin, height - margin)
            : Phaser.Math.Clamp(this.sprite.y + Phaser.Math.Between(60, 120), margin, height - margin);
    ...
    startChase(player) {
        ...
        this.isChasing = true;
        this.setWalk();
        this.scene.physics.moveToObject(this.sprite, player.sprite, stats.moveSpeed);
    }
    ...
    think(player) {
        ...
        if (dist >= BOSS1_ATTACK_RANGE_MIN && dist <= BOSS1_ATTACK_RANGE_MAX) {
            ...
            } else if (roll < stats.attackWeight + stats.walkAttackWeight + stats.repositionWeight) {
                this.reposition(player, Math.random() > 0.5);
            } else {
                this.fireAtPlayer(player, true);
            }
    ...
    update(player) {
        ...
        if (this.isChasing) {
            ...
            if (dist <= BOSS1_ATTACK_RANGE_MAX) {
                this.stopMovement();
                this.setIdle();
                if (dist >= BOSS1_ATTACK_RANGE_MIN && this.scene.time.now >= this.lastFireAt + stats.fireCooldown * 0.5) {
                    this.fireAtPlayer(player, false);
                }
            } else {
                this.scene.physics.moveToObject(this.sprite, player.sprite, stats.moveSpeed);
